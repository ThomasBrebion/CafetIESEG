...
public class SupprimerSandwichServlet extends GenericServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Sandwich> sandwich = Ensemble.getInstance().listerSandwichs();
		request.setAttribute("listeSandwich", sandwich);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (this.isNullOrEmpty(nom)) {
			...
			response.sendRedirect("supprimerSandwich");
			} else {
			Ensemble.getInstance().supprimerSandwich(nom);
